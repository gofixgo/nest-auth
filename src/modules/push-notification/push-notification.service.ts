...
import { ISendPushNotificationToOneUser } from './push-notification.type';
...
export class PushNotificationService {
  ...
  async subscribe(createDto: CreatePushNotificationDto, user: IUserAuth) {
    return await this.deviceService.create({ ...createDto, user_id: user.user_id });
  }
  ...
  async sendToUsers(payload: ISendPushNotificationToOneUser) {
    const devices: Device[] = [];
    if (payload.ids) {
      const users = await this.em.find(User, { user_id: { $in: payload.ids } }, { populate: ['user_devices'] });
  ...
  async getManyUsersDevices(roles_ids: string[] | string) {
    ...
    const devices: Device[] = [];
    const users = await this.em.find(User, { user_role_ids: { $contains: resolvedRoles } }, { populate: ['user_devices'] });
    for await (const u of users) {
      const usersDevices = await u.user_devices.loadItems();
      devices.push(...usersDevices);
    }
  ...
  async getOneUserDevices(role_id: string) {
    const user = await this.em.findOne(User, { user_role_ids: { $contains: [role_id] } }, { populate: ['user_devices'] });
